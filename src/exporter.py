...
def generate_interface():
    ...
    for prop, typ in opcode_output.items():
        relname = "".join(a[0].upper() + a[1:] for a in ("opcode_" + prop).split("_"))
        if typ is bool:
    ...
    for prop, typ in opcode_output.items():
        relname = "".join(a[0].upper() + a[1:] for a in ("opcode_" + prop).split("_"))
        for _k, opcode in opcodes.OPCODES.items():
...
class EVMBlockExporter(FactExporter):
    ...
    def process_metadata(self, metadata: dict[Any, Any] | None = None) -> None:
        ...
        self.function_debug_data = (
            process_function_debug_data(metadata.get("function_debug_info", {})) if metadata is not None else []
        )
        self.immutable_references = (
            process_immutable_refs(metadata.get("immutable_references", {})) if metadata is not None else []
        )
    ...
    def export(self):
        ...
        self.generate("Statement_Next.facts", pairwise(instructions_order))
