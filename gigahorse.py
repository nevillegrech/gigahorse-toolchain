...
import src.source_map as source_map
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Assume that source mapping is provided with bytecode.",
)
...
def analyze_contract(
    ...
    def run_clients(souffle_clients, other_clients, in_dir, out_dir):
        ...
        for souffle_client in souffle_clients:
            ...
            if (
                run_process(
                    analysis_args, calc_timeout(souffle_client), stderr=err_file
                )
                < 0
            ):
    ...
    try:
        ...
        if exists:
            ...
            if args.has_source and os.path.exists(
                ...
                raise RuntimeError(
                    f"Gigahorse is running in source mode, but contract {contract_filename} does not have a source mapping."
                )
            ...
            if args.context_depth is not None:
                write_context_depth_file(
                    os.path.join(work_dir, "MaxContextDepth.csv"), args.context_depth
                )
