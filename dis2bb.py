...
# Construct a list of DisasmLine objects from the raw input disassembly lines,
# ignoring the first line of input (which is the bytecode's hex representation
# when using Ethereum's disasm tool). Any line which does not produce enough
# tokens to be valid disassembly after being split() is also ignored.
lines = [
...
# (Used for mapping jump destinations to actual disassembly lines)
...
# List of DisasmLines which represent JUMPs with an unresolved destination address
# Either these jumps use computed addresses, or the destination is pushed to
# the stack in a previous block.
unresolved_jumps = []

# List of BasicBlock objects making up our CFG
blocks = []
