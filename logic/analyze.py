...
c = statsd.StatsClient('18.237.49.242', 8125, prefix='analyze')
...
try:
    while not contracts_exhausted:
        c.gauge("available-jobs.int", len(avail_jobs))
        ...
        while not contracts_exhausted and len(avail_jobs) > 0:
            try:
                ...
                workers.append({"name": contract_name,
                                ...
                                "time": start_time,
                                "job_index": job_index})
                c.gauge("contracts-exhausted.int", 0)
            except StopIteration:
                contracts_exhausted = True
                c.gauge("contracts-exhausted.int", 1)
        ...
        while len(avail_jobs) == 0 or (contracts_exhausted and 0 < len(workers)):
            ...
            for i in range(len(workers)):
                ...
                if time.time() - start_time > (args.timeout_secs + 1):
                    ...
                    proc.join()
                    avail_jobs.append(job_index)
                    c.incr("process-terminated.int", 1)
    ...
    c.incr("total-flagged-contracts.int", total_flagged)
    c.incr("total-contracts.int", total)
